-- 021_merge_renovations.sql
-- Fold the 020 tables (resume_renovations, resume_renovation_versions,
-- usage_events) into the cross-device merge. Without this, a device that built
-- renovations while anonymous and then signed into an existing account would
-- strand them under the tombstoned source uid (invisible under RLS) — exactly
-- the orphaning 017/018 exist to prevent.
--
-- This CREATE OR REPLACE reproduces migration 018's redeem_merge_grant(uuid,text)
-- body VERBATIM and only appends three table moves before the tombstone step.
-- The 1-arg delegate (redeem_merge_grant(uuid)) resolves the 2-arg by name at
-- call time, so it picks up this body automatically — no need to redefine it.
-- mint_merge_grant and all grants/REVOKEs are unchanged from 018. The Flow B
-- test harness re-runs every 017/018 scenario against this replaced function,
-- so a transcription error fails CI rather than silently shipping.
...
BEGIN
  ...
  -- ---- per-table merge (byte-identical to 018) --------------------------
...
-- state the invariant explicitly, matching 018's footer).
