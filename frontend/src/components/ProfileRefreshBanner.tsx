...
export default function ProfileRefreshBanner({ refresh, targetReady = true, locale = 'en', onBeforeReview }: {
  refresh?: ProfileRefreshState; targetReady?: boolean; locale?: 'en' | 'zh';
  onBeforeReview?: () => boolean;
}) {
  if (targetReady && (!refresh || refresh.status === 'ready')) return null;
  ...
  return <div role={status === 'failed' || status === 'conflict' ? 'alert' : 'status'}
    ...
    {status === 'local-only' && <p>{copy('Using this device’s profile. Cloud updates are unavailable.', '正在使用本机资料，暂不能读取云端更新。')}</p>}
    {!targetReady && <p>{copy('This target is not confirmed in the current results. Your draft is kept; generation and outreach are paused.', '当前结果暂未确认此目标。草稿仍保留，生成和邮件操作已暂停。')}</p>}
