  ...
  return (
    <section className="mt-8 mb-4" aria-labelledby="similar-heading">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {similar.map((s) => {
          const isFaculty = s.source_type === 'faculty_research';
          return <li key={s.id}>
            <Link
            ...
            >
              <div className="flex items-start gap-2 mb-2">
                ...
                {!isFaculty && s.paid === 'yes' && <DetailBadge tone="emerald">{t('badges.paid')}</DetailBadge>}
                {!isFaculty && s.paid === 'stipend' && <DetailBadge tone="emerald">{t('badges.stipend')}</DetailBadge>}
              </div>
