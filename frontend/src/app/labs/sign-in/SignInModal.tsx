...
/*
 * Labs prototype — the redesigned sign-in MODAL (chosen direction).
 *
 * Eric's review decision: centered variant won, presented as a modal
 * over the current page ("弹窗出来在页面上，不要跳到新页面"). Production
 * path: this design upgrades the existing AuthModal signin phase;
 * /auth/sign-in stays only as a deep-link fallback that opens it.
 *
 * Modal chrome copies the project's modal vocabulary verbatim
 * (AuthModal / SaveSearchDialog / KeyboardHelpDialog):
 *   fixed inset-0 z-[60] flex items-center justify-center p-4
 *   absolute inset-0 bg-gray-900/60 backdrop-blur-sm
 *   relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 animate-in
 * plus Escape-to-close and the KeyboardHelpDialog body-scroll lock.
 *
 * Inside the card, the page hero compresses: the headline becomes the
 * modal title block, and the three value props collapse into a compact
 * icon+label row (SaveSearchDialog's gray-50/80 inset-box idiom).
 *
 * Nothing talks to Supabase: both provider buttons are disabled visuals
 * with a "design preview" tooltip, and "Send magic link" flips to a
 * mocked sent panel. Strings are plain bilingual on purpose —
...
const VALUE_PROPS = [
  { icon: MonitorSmartphone, zh: '全设备同步', en: 'Sync everywhere' },
  { icon: ShieldCheck, zh: '数据不丢失', en: 'Never lose work' },
  { icon: Inbox, zh: '每周新匹配', en: 'Weekly matches' },
] as const;
  ...
  return (
    <div className="relative group">
      <button
        ...
        className="w-full flex items-center justify-center gap-2.5 py-2.5 px-3 rounded-xl border border-gray-200 bg-white text-[14px] font-medium text-gray-700 leading-snug text-center cursor-not-allowed"
      >
        {logo}
        <span>{label}</span>
      </button>
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md max-h-[calc(100vh-2rem)] overflow-y-auto bg-white rounded-2xl shadow-2xl p-6 animate-in">
        ...
        {sent ? (
          <div data-testid="sent-panel">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center shrink-0">
                <CheckCircle className="w-5 h-5 text-emerald-600" aria-hidden="true" />
              </div>
              <div className="min-w-0">
                <h2 id="signin-modal-title" className="text-[16px] font-semibold text-gray-900">
                  Check your inbox
                </h2>
          ...
          <>
            {/* Hero headline, compressed into the modal title block. */}
            <div className="pr-6">
              <h2
            ...
            {/* The three value props, compressed to an icon+label band. */}
            <div className="mt-4 grid grid-cols-3 gap-2 rounded-xl border border-gray-100 bg-gray-50/80 px-3 py-2.5">
              {VALUE_PROPS.map(p => (
                <div key={p.en} className="flex flex-col items-center text-center gap-1">
                  <p.icon className="w-4 h-4 text-blue-600" aria-hidden="true" />
                  <p className="text-[11px] font-medium text-gray-700 leading-tight">{p.zh}</p>
                  <p className="text-[11px] text-gray-400 leading-tight">{p.en}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 space-y-2.5">
              <ProviderButton
                tipId="google-preview-tip"
                logo={<GoogleLogo />}
                label="Continue with Google"
              />
              <ProviderButton
                tipId="microsoft-preview-tip"
                logo={<MicrosoftLogo />}
                label="用学校账号登录 · School account (Microsoft)"
              />
            </div>
