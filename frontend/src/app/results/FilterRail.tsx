...
export interface FilterRailProps {
  ...
  /** True when this page cannot pair the displayed value with the snapshot
   *  a click would write against. Disabled beats guessing: the alternative is
   *  sending a flip the person made against values they were not shown. */
  crossSchoolDisabled: boolean;
  /** The last flip did not reach storage. Shown, never swallowed: a toggle
   *  that snaps back with no explanation reads as a broken control, and one
   *  that stays flipped is a lie about what was saved. */
  crossSchoolFailed: boolean;
  onCrossSchoolRetry: () => void;
      ...
      >
        ...
        {RELEASE_SCOPE.crossSchoolMatching && (
          <button
            ...
            disabled={crossSchoolDisabled}
            aria-pressed={includeCrossSchool}
            title={t('results.filters.crossSchoolHint')}
            className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-[12px] font-medium border transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              includeCrossSchool
        ...
        {RELEASE_SCOPE.crossSchoolMatching && crossSchoolFailed && (
          <span data-testid="cross-school-failed" className="inline-flex items-center gap-1 text-[12px] text-red-600">
            {t('results.filters.crossSchoolFailed')}
            <button
              type="button"
              onClick={onCrossSchoolRetry}
