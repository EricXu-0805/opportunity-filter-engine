/*
 * Single school registry: the .edu domain → school map used by
 * AuthModal's live email detection chip, plus the switcher-facing
 * metadata (location, coverage) consumed by UniversitySwitcherModal
 * and the results scope indicator. One list, no parallel copies.
 */

/*
 * Per-school corpus counts, generated by scripts/gen_school_stats.py — which
 * calls the same backend function /api/opportunities/coverage calls, so the
 * static fallback below and the live number mean the same thing. The `prebuild`
 * hook validates the committed file's schema; it does not recount (see
 * frontend/scripts/gen-school-stats.mjs for why). The corpus itself never
 * enters the bundle.
 */
import schoolStats from './school-stats.json';
import type { SchoolCoverageCounts, SchoolStatsFile } from './school-coverage';

export interface SchoolCoverage {
  /**
   * This school's coverage — unique listings PLUS unique faculty contacts, the
   * product's one definition (backend/lib/school_coverage.py) — or 'pending'
   * when nothing has been collected for the campus yet (the school still sees
   * every national `audience='open'` record).
   *
   * Faculty contacts are ~97% of the corpus, so this deliberately is not a
   * listings count: reading only that half is the bug this field's comment now
   * exists to prevent, and it understated schools by up to 165x.
   *
   * Floored to a friendly "N+" display so the chip never overstates. It is the
   * build-time fallback only; the live value comes from
   * /api/opportunities/coverage and carries the same definition, so replacing
   * one with the other shifts the number by data freshness, never by meaning.
   */
  campusOpportunities: number | 'pending';
  /** i18n key for the coverage chip text shown on the switcher card. */
  note: string;
}

export interface School {
  slug: string;
  /** Primary email domain. Subdomains match too (cs.illinois.edu). */
  domain: string;
  name: string;
  shortName: string;
  nameZh: string;
  /** Official brand color, used for the chip dot. */
  color: string;
  location: string;
  coverage: SchoolCoverage;
  /**
   * College/major catalog counts shown on the switcher card, or null for
   * schools whose catalog hasn't shipped (those fall back to free-text
   * college/major inputs). Static so the cards don't pull every catalog
   * chunk into the main bundle; catalogs.test.ts asserts these numbers
   * exactly match the data behind loadCatalog().
   */
  catalog: { colleges: number; majors: number } | null;
}

const SCHOOL_STATS_FILE = schoolStats as SchoolStatsFile;

/** Per-school static counts, keyed by slug. Absent slug = nothing collected. */
export const SCHOOL_STATS: Record<string, SchoolCoverageCounts | undefined> =
  SCHOOL_STATS_FILE.schools;

/**
 * Actionable records open to every school regardless of campus. Reported in the
 * switcher footer, never added into a card's own number: a pool every school
 * shares would overstate each card and, summed across the grid, count itself
 * once per school.
 */
export const NATIONAL_OPPORTUNITY_COUNT = SCHOOL_STATS_FILE.national_count;

/*
 * The chip counts only this school's own campus-hosted records. The national
 * `audience='open'` pool is shared by every school, so folding it into any one
 * card's number both overstates that card and reads inconsistently across the
 * grid — it's explained once in the switcher footer instead.
 */
function campusCoverage(slug: string): SchoolCoverage {
  const stat = SCHOOL_STATS[slug];
  if (!stat?.total_count) {
    return { campusOpportunities: 'pending', note: 'universitySwitcher.coveragePending' };
  }
  return {
    campusOpportunities: stat.total_count,
    note: 'universitySwitcher.coverageCampus',
  };
}

export const SCHOOLS: School[] = [
  {
    slug: 'princeton',
    domain: 'princeton.edu',
    name: 'Princeton University',
    shortName: 'Princeton',
    nameZh: '普林斯顿大学',
    color: '#E77500',
    location: 'Princeton, NJ',
    coverage: campusCoverage('princeton'),
    catalog: { colleges: 5, majors: 38 },
  },
  {
    slug: 'mit',
    domain: 'mit.edu',
    name: 'Massachusetts Institute of Technology',
    shortName: 'MIT',
    nameZh: '麻省理工学院',
    color: '#A31F34',
    location: 'Cambridge, MA',
    coverage: campusCoverage('mit'),
    catalog: { colleges: 6, majors: 36 },
  },
  {
    slug: 'harvard',
    domain: 'harvard.edu',
    name: 'Harvard University',
    shortName: 'Harvard',
    nameZh: '哈佛大学',
    color: '#A51C30',
    location: 'Cambridge, MA',
    coverage: campusCoverage('harvard'),
    catalog: { colleges: 4, majors: 39 },
  },
  {
    slug: 'stanford',
    domain: 'stanford.edu',
    name: 'Stanford University',
    shortName: 'Stanford',
    nameZh: '斯坦福大学',
    color: '#8C1515',
    location: 'Stanford, CA',
    coverage: campusCoverage('stanford'),
    catalog: { colleges: 3, majors: 72 },
  },
  {
    slug: 'yale',
    domain: 'yale.edu',
    name: 'Yale University',
    shortName: 'Yale',
    nameZh: '耶鲁大学',
    color: '#00356B',
    location: 'New Haven, CT',
    coverage: campusCoverage('yale'),
    catalog: { colleges: 4, majors: 62 },
  },
  {
    slug: 'uchicago',
    domain: 'uchicago.edu',
    name: 'University of Chicago',
    shortName: 'UChicago',
    nameZh: '芝加哥大学',
    color: '#800000',
    location: 'Chicago, IL',
    coverage: campusCoverage('uchicago'),
    catalog: { colleges: 4, majors: 56 },
  },
  {
    slug: 'duke',
    domain: 'duke.edu',
    name: 'Duke University',
    shortName: 'Duke',
    nameZh: '杜克大学',
    color: '#00539B',
    location: 'Durham, NC',
    coverage: campusCoverage('duke'),
    catalog: { colleges: 2, majors: 25 },
  },
  {
    slug: 'jhu',
    domain: 'jhu.edu',
    name: 'Johns Hopkins University',
    shortName: 'Johns Hopkins',
    nameZh: '约翰斯·霍普金斯大学',
    color: '#002D72',
    location: 'Baltimore, MD',
    coverage: campusCoverage('jhu'),
    catalog: { colleges: 7, majors: 52 },
  },
  {
    slug: 'northwestern',
    domain: 'northwestern.edu',
    name: 'Northwestern University',
    shortName: 'Northwestern',
    nameZh: '西北大学',
    color: '#4E2A84',
    location: 'Evanston, IL',
    coverage: campusCoverage('northwestern'),
    catalog: { colleges: 7, majors: 48 },
  },
  {
    slug: 'upenn',
    domain: 'upenn.edu',
    name: 'University of Pennsylvania',
    shortName: 'Penn',
    nameZh: '宾夕法尼亚大学',
    color: '#011F5B',
    location: 'Philadelphia, PA',
    coverage: campusCoverage('upenn'),
    catalog: { colleges: 4, majors: 66 },
  },
  {
    slug: 'caltech',
    domain: 'caltech.edu',
    name: 'California Institute of Technology',
    shortName: 'Caltech',
    nameZh: '加州理工学院',
    color: '#FF6C0C',
    location: 'Pasadena, CA',
    coverage: campusCoverage('caltech'),
    catalog: { colleges: 6, majors: 28 },
  },
  {
    slug: 'cornell',
    domain: 'cornell.edu',
    name: 'Cornell University',
    shortName: 'Cornell',
    nameZh: '康奈尔大学',
    color: '#B31B1B',
    location: 'Ithaca, NY',
    coverage: campusCoverage('cornell'),
    catalog: { colleges: 9, majors: 99 },
  },
  {
    slug: 'brown',
    domain: 'brown.edu',
    name: 'Brown University',
    shortName: 'Brown',
    nameZh: '布朗大学',
    color: '#4E3629',
    location: 'Providence, RI',
    coverage: campusCoverage('brown'),
    catalog: { colleges: 7, majors: 88 },
  },
  {
    slug: 'dartmouth',
    domain: 'dartmouth.edu',
    name: 'Dartmouth College',
    shortName: 'Dartmouth',
    nameZh: '达特茅斯学院',
    color: '#00693E',
    location: 'Hanover, NH',
    coverage: campusCoverage('dartmouth'),
    catalog: { colleges: 5, majors: 41 },
  },
  {
    slug: 'columbia',
    domain: 'columbia.edu',
    name: 'Columbia University',
    shortName: 'Columbia',
    nameZh: '哥伦比亚大学',
    color: '#0077C8',
    location: 'New York, NY',
    coverage: campusCoverage('columbia'),
    catalog: { colleges: 3, majors: 65 },
  },
  {
    slug: 'ucb',
    domain: 'berkeley.edu',
    name: 'University of California, Berkeley',
    shortName: 'UC Berkeley',
    nameZh: '加州大学伯克利分校',
    color: '#003262',
    location: 'Berkeley, CA',
    coverage: campusCoverage('ucb'),
    catalog: { colleges: 7, majors: 136 },
  },
  {
    slug: 'rice',
    domain: 'rice.edu',
    name: 'Rice University',
    shortName: 'Rice',
    nameZh: '莱斯大学',
    color: '#00205B',
    location: 'Houston, TX',
    coverage: campusCoverage('rice'),
    catalog: { colleges: 7, majors: 62 },
  },
  {
    slug: 'ucla',
    domain: 'ucla.edu',
    name: 'University of California, Los Angeles',
    shortName: 'UCLA',
    nameZh: '加州大学洛杉矶分校',
    color: '#2774AE',
    location: 'Los Angeles, CA',
    coverage: campusCoverage('ucla'),
    catalog: { colleges: 9, majors: 135 },
  },
  {
    slug: 'vanderbilt',
    domain: 'vanderbilt.edu',
    name: 'Vanderbilt University',
    shortName: 'Vanderbilt',
    nameZh: '范德堡大学',
    color: '#866D4B',
    location: 'Nashville, TN',
    coverage: campusCoverage('vanderbilt'),
    catalog: { colleges: 4, majors: 66 },
  },
  {
    slug: 'cmu',
    domain: 'cmu.edu',
    name: 'Carnegie Mellon University',
    shortName: 'CMU',
    nameZh: '卡内基梅隆大学',
    color: '#C41230',
    location: 'Pittsburgh, PA',
    coverage: campusCoverage('cmu'),
    catalog: { colleges: 6, majors: 37 },
  },
  {
    slug: 'umich',
    domain: 'umich.edu',
    name: 'University of Michigan',
    shortName: 'Michigan',
    nameZh: '密歇根大学',
    color: '#00274C',
    location: 'Ann Arbor, MI',
    coverage: campusCoverage('umich'),
    catalog: { colleges: 14, majors: 127 },
  },
  {
    slug: 'nd',
    domain: 'nd.edu',
    name: 'University of Notre Dame',
    shortName: 'Notre Dame',
    nameZh: '圣母大学',
    color: '#0C2340',
    location: 'Notre Dame, IN',
    coverage: campusCoverage('nd'),
    catalog: { colleges: 6, majors: 34 },
  },
  {
    slug: 'washu',
    domain: 'wustl.edu',
    name: 'Washington University in St. Louis',
    shortName: 'WashU',
    nameZh: '圣路易斯华盛顿大学',
    color: '#A51417',
    location: 'St. Louis, MO',
    coverage: campusCoverage('washu'),
    catalog: { colleges: 4, majors: 35 },
  },
  {
    slug: 'usc',
    domain: 'usc.edu',
    name: 'University of Southern California',
    shortName: 'USC',
    nameZh: '南加州大学',
    color: '#990000',
    location: 'Los Angeles, CA',
    coverage: campusCoverage('usc'),
    catalog: { colleges: 14, majors: 48 },
  },
  {
    slug: 'ucsd',
    domain: 'ucsd.edu',
    name: 'University of California, San Diego',
    shortName: 'UC San Diego',
    nameZh: '加州大学圣地亚哥分校',
    color: '#182B49',
    location: 'La Jolla, CA',
    coverage: campusCoverage('ucsd'),
    catalog: { colleges: 8, majors: 165 },
  },
  {
    slug: 'utexas',
    domain: 'utexas.edu',
    name: 'The University of Texas at Austin',
    shortName: 'UT Austin',
    nameZh: '得克萨斯大学奥斯汀分校',
    color: '#BF5700',
    location: 'Austin, TX',
    coverage: campusCoverage('utexas'),
    catalog: { colleges: 14, majors: 113 },
  },
  {
    slug: 'uf',
    domain: 'ufl.edu',
    name: 'University of Florida',
    shortName: 'UF',
    nameZh: '佛罗里达大学',
    color: '#0021A5',
    location: 'Gainesville, FL',
    coverage: campusCoverage('uf'),
    catalog: { colleges: 12, majors: 53 },
  },
  {
    slug: 'gatech',
    domain: 'gatech.edu',
    name: 'Georgia Institute of Technology',
    shortName: 'Georgia Tech',
    nameZh: '佐治亚理工学院',
    color: '#B3A369',
    location: 'Atlanta, GA',
    coverage: campusCoverage('gatech'),
    catalog: { colleges: 6, majors: 43 },
  },
  {
    slug: 'uci',
    domain: 'uci.edu',
    name: 'University of California, Irvine',
    shortName: 'UC Irvine',
    nameZh: '加州大学欧文分校',
    color: '#0064A4',
    location: 'Irvine, CA',
    coverage: campusCoverage('uci'),
    catalog: { colleges: 14, majors: 84 },
  },
  {
    slug: 'uiuc',
    domain: 'illinois.edu',
    name: 'University of Illinois Urbana-Champaign',
    shortName: 'UIUC',
    nameZh: '伊利诺伊大学香槟分校',
    color: '#E84A27',
    location: 'Urbana-Champaign, IL',
    coverage: campusCoverage('uiuc'),
    catalog: { colleges: 12, majors: 142 },
  },
  {
    slug: 'wisc',
    domain: 'wisc.edu',
    name: 'University of Wisconsin–Madison',
    shortName: 'UW–Madison',
    nameZh: '威斯康星大学麦迪逊分校',
    color: '#C5050C',
    location: 'Madison, WI',
    coverage: campusCoverage('wisc'),
    catalog: { colleges: 8, majors: 146 },
  },
  {
    slug: 'ucsb',
    domain: 'ucsb.edu',
    name: 'University of California, Santa Barbara',
    shortName: 'UC Santa Barbara',
    nameZh: '加州大学圣塔芭芭拉分校',
    color: '#003660',
    location: 'Santa Barbara, CA',
    coverage: campusCoverage('ucsb'),
    catalog: { colleges: 3, majors: 73 },
  },
  {
    slug: 'osu',
    domain: 'osu.edu',
    name: 'Ohio State University',
    shortName: 'Ohio State',
    nameZh: '俄亥俄州立大学',
    color: '#BB0000',
    location: 'Columbus, OH',
    coverage: campusCoverage('osu'),
    catalog: { colleges: 12, majors: 67 },
  },
  {
    slug: 'bu',
    domain: 'bu.edu',
    name: 'Boston University',
    shortName: 'BU',
    nameZh: '波士顿大学',
    color: '#CC0000',
    location: 'Boston, MA',
    coverage: campusCoverage('bu'),
    catalog: { colleges: 11, majors: 45 },
  },
  {
    slug: 'rutgers',
    domain: 'rutgers.edu',
    name: 'Rutgers University-New Brunswick',
    shortName: 'Rutgers',
    nameZh: '罗格斯大学新布朗斯维克分校',
    color: '#CC0033',
    location: 'New Brunswick, NJ',
    coverage: campusCoverage('rutgers'),
    catalog: { colleges: 10, majors: 53 },
  },
  {
    slug: 'umd',
    domain: 'umd.edu',
    name: 'University of Maryland, College Park',
    shortName: 'Maryland',
    nameZh: '马里兰大学帕克分校',
    color: '#E21833',
    location: 'College Park, MD',
    coverage: campusCoverage('umd'),
    catalog: { colleges: 12, majors: 63 },
  },
  {
    slug: 'uw',
    domain: 'washington.edu',
    name: 'University of Washington',
    shortName: 'UW',
    nameZh: '华盛顿大学',
    color: '#4B2E83',
    location: 'Seattle, WA',
    coverage: campusCoverage('uw'),
    catalog: { colleges: 12, majors: 116 },
  },
  {
    slug: 'lehigh',
    domain: 'lehigh.edu',
    name: 'Lehigh University',
    shortName: 'Lehigh',
    nameZh: '里海大学',
    color: '#653819',
    location: 'Bethlehem, PA',
    coverage: campusCoverage('lehigh'),
    catalog: { colleges: 4, majors: 43 },
  },
  {
    slug: 'neu',
    domain: 'northeastern.edu',
    name: 'Northeastern University',
    shortName: 'Northeastern',
    nameZh: '美国东北大学',
    color: '#D41B2C',
    location: 'Boston, MA',
    coverage: campusCoverage('neu'),
    catalog: { colleges: 7, majors: 48 },
  },
  {
    slug: 'purdue',
    domain: 'purdue.edu',
    name: 'Purdue University',
    shortName: 'Purdue',
    nameZh: '普渡大学',
    color: '#CEB888',
    location: 'West Lafayette, IN',
    coverage: campusCoverage('purdue'),
    catalog: { colleges: 10, majors: 70 },
  },
  {
    slug: 'uga',
    domain: 'uga.edu',
    name: 'University of Georgia',
    shortName: 'UGA',
    nameZh: '佐治亚大学',
    color: '#BA0C2F',
    location: 'Athens, GA',
    coverage: campusCoverage('uga'),
    catalog: { colleges: 14, majors: 97 },
  },
  {
    slug: 'rochester',
    domain: 'rochester.edu',
    name: 'University of Rochester',
    shortName: 'Rochester',
    nameZh: '罗切斯特大学',
    color: '#003B71',
    location: 'Rochester, NY',
    coverage: campusCoverage('rochester'),
    catalog: { colleges: 4, majors: 22 },
  },
  {
    slug: 'casewestern',
    domain: 'case.edu',
    name: 'Case Western Reserve University',
    shortName: 'Case Western',
    nameZh: '凯斯西储大学',
    color: '#0A304E',
    location: 'Cleveland, OH',
    coverage: campusCoverage('casewestern'),
    catalog: { colleges: 4, majors: 39 },
  },
  {
    slug: 'fsu',
    domain: 'fsu.edu',
    name: 'Florida State University',
    shortName: 'FSU',
    nameZh: '佛罗里达州立大学',
    color: '#782F40',
    location: 'Tallahassee, FL',
    coverage: campusCoverage('fsu'),
    catalog: { colleges: 10, majors: 62 },
  },
  {
    slug: 'tamu',
    domain: 'tamu.edu',
    name: 'Texas A&M University',
    shortName: 'Texas A&M',
    nameZh: '得克萨斯农工大学',
    color: '#500000',
    location: 'College Station, TX',
    coverage: campusCoverage('tamu'),
    catalog: { colleges: 12, majors: 65 },
  },
  {
    slug: 'vt',
    domain: 'vt.edu',
    name: 'Virginia Tech',
    shortName: 'Virginia Tech',
    nameZh: '弗吉尼亚理工',
    color: '#861F41',
    location: 'Blacksburg, VA',
    coverage: campusCoverage('vt'),
    catalog: { colleges: 7, majors: 61 },
  },
  {
    slug: 'psu',
    domain: 'psu.edu',
    name: 'Penn State University Park',
    shortName: 'Penn State',
    nameZh: '宾夕法尼亚州立大学',
    color: '#041E42',
    location: 'University Park, PA',
    coverage: campusCoverage('psu'),
    catalog: { colleges: 12, majors: 73 },
  },
  {
    slug: 'sbu',
    domain: 'stonybrook.edu',
    name: 'Stony Brook University',
    shortName: 'Stony Brook',
    nameZh: '石溪大学',
    color: '#990000',
    location: 'Stony Brook, NY',
    coverage: campusCoverage('sbu'),
    catalog: { colleges: 8, majors: 39 },
  },
  {
    slug: 'umn',
    domain: 'umn.edu',
    name: 'University of Minnesota Twin Cities',
    shortName: 'UMN',
    nameZh: '明尼苏达大学双城分校',
    color: '#7A0019',
    location: 'Minneapolis, MN',
    coverage: campusCoverage('umn'),
    catalog: { colleges: 9, majors: 67 },
  },
  {
    slug: 'msu',
    domain: 'msu.edu',
    name: 'Michigan State University',
    shortName: 'Michigan State',
    nameZh: '密歇根州立大学',
    color: '#18453B',
    location: 'East Lansing, MI',
    coverage: campusCoverage('msu'),
    catalog: { colleges: 10, majors: 78 },
  },
  {
    slug: 'ncsu',
    domain: 'ncsu.edu',
    name: 'North Carolina State University',
    shortName: 'NC State',
    nameZh: '北卡罗来纳州立大学',
    color: '#CC0000',
    location: 'Raleigh, NC',
    coverage: campusCoverage('ncsu'),
    catalog: { colleges: 9, majors: 59 },
  },
  {
    slug: 'rpi',
    domain: 'rpi.edu',
    name: 'Rensselaer Polytechnic Institute',
    shortName: 'RPI',
    nameZh: '伦斯勒理工学院',
    color: '#D6001C',
    location: 'Troy, NY',
    coverage: campusCoverage('rpi'),
    catalog: { colleges: 5, majors: 42 },
  },
  {
    slug: 'umass',
    domain: 'umass.edu',
    name: 'University of Massachusetts Amherst',
    shortName: 'UMass',
    nameZh: '马萨诸塞大学阿默斯特分校',
    color: '#881C1C',
    location: 'Amherst, MA',
    coverage: campusCoverage('umass'),
    catalog: { colleges: 10, majors: 79 },
  },
  {
    slug: 'miami',
    domain: 'miami.edu',
    name: 'University of Miami',
    shortName: 'Miami',
    nameZh: '迈阿密大学',
    color: '#005030',
    location: 'Coral Gables, FL',
    coverage: campusCoverage('miami'),
    catalog: { colleges: 7, majors: 55 },
  },
  {
    slug: 'uconn',
    domain: 'uconn.edu',
    name: 'University of Connecticut',
    shortName: 'UConn',
    nameZh: '康涅狄格大学',
    color: '#000E2F',
    location: 'Storrs, CT',
    coverage: campusCoverage('uconn'),
    catalog: { colleges: 6, majors: 41 },
  },
  {
    slug: 'pitt',
    domain: 'pitt.edu',
    name: 'University of Pittsburgh',
    shortName: 'Pitt',
    nameZh: '匹兹堡大学',
    color: '#003594',
    location: 'Pittsburgh, PA',
    coverage: campusCoverage('pitt'),
    catalog: { colleges: 8, majors: 63 },
  },
  {
    slug: 'indiana',
    domain: 'indiana.edu',
    name: 'Indiana University Bloomington',
    shortName: 'Indiana',
    nameZh: '印第安纳大学伯明顿分校',
    color: '#990000',
    location: 'Bloomington, IN',
    coverage: campusCoverage('indiana'),
    catalog: { colleges: 7, majors: 38 },
  },
  {
    slug: 'clemson',
    domain: 'clemson.edu',
    name: 'Clemson University',
    shortName: 'Clemson',
    nameZh: '克莱姆森大学',
    color: '#F56600',
    location: 'Clemson, SC',
    coverage: campusCoverage('clemson'),
    catalog: { colleges: 6, majors: 54 },
  },
  {
    slug: 'syracuse',
    domain: 'syr.edu',
    name: 'Syracuse University',
    shortName: 'Syracuse',
    nameZh: '雪城大学',
    color: '#F76900',
    location: 'Syracuse, NY',
    coverage: campusCoverage('syracuse'),
    catalog: { colleges: 5, majors: 41 },
  },
  {
    slug: 'buffalo',
    domain: 'buffalo.edu',
    name: 'University at Buffalo (SUNY)',
    shortName: 'UB',
    nameZh: '纽约州立大学布法罗分校',
    color: '#005BBB',
    location: 'Buffalo, NY',
    coverage: campusCoverage('buffalo'),
    catalog: { colleges: 6, majors: 41 },
  },
  {
    slug: 'ucr',
    domain: 'ucr.edu',
    name: 'University of California, Riverside',
    shortName: 'UC Riverside',
    nameZh: '加州大学河滨分校',
    color: '#003DA5',
    location: 'Riverside, CA',
    coverage: campusCoverage('ucr'),
    catalog: { colleges: 6, majors: 57 },
  },
  {
    slug: 'drexel',
    domain: 'drexel.edu',
    name: 'Drexel University',
    shortName: 'Drexel',
    nameZh: '德雷塞尔大学',
    color: '#07294D',
    location: 'Philadelphia, PA',
    coverage: campusCoverage('drexel'),
    catalog: { colleges: 6, majors: 47 },
  },
  {
    slug: 'njit',
    domain: 'njit.edu',
    name: 'New Jersey Institute of Technology',
    shortName: 'NJIT',
    nameZh: '新泽西理工学院',
    color: '#D31245',
    location: 'Newark, NJ',
    coverage: campusCoverage('njit'),
    catalog: { colleges: 5, majors: 37 },
  },
  {
    slug: 'stevens',
    domain: 'stevens.edu',
    name: 'Stevens Institute of Technology',
    shortName: 'Stevens',
    nameZh: '史蒂文斯理工学院',
    color: '#A32638',
    location: 'Hoboken, NJ',
    coverage: campusCoverage('stevens'),
    catalog: { colleges: 4, majors: 36 },
  },
  {
    slug: 'wpi',
    domain: 'wpi.edu',
    name: 'Worcester Polytechnic Institute',
    shortName: 'WPI',
    nameZh: '伍斯特理工学院',
    color: '#A9002A',
    location: 'Worcester, MA',
    coverage: campusCoverage('wpi'),
    catalog: { colleges: 4, majors: 34 },
  },
  {
    slug: 'ucsc',
    domain: 'ucsc.edu',
    name: 'University of California, Santa Cruz',
    shortName: 'UC Santa Cruz',
    nameZh: '加州大学圣克鲁兹分校',
    color: '#003C6C',
    location: 'Santa Cruz, CA',
    coverage: campusCoverage('ucsc'),
    catalog: { colleges: 5, majors: 49 },
  },
  {
    slug: 'udel',
    domain: 'udel.edu',
    name: 'University of Delaware',
    shortName: 'Delaware',
    nameZh: '特拉华大学',
    color: '#00539F',
    location: 'Newark, DE',
    coverage: campusCoverage('udel'),
    catalog: { colleges: 6, majors: 51 },
  },
  {
    slug: 'usf',
    domain: 'usf.edu',
    name: 'University of South Florida',
    shortName: 'USF',
    nameZh: '南佛罗里达大学',
    color: '#006747',
    location: 'Tampa, FL',
    coverage: campusCoverage('usf'),
    catalog: { colleges: 7, majors: 36 },
  },
  {
    slug: 'boulder',
    domain: 'colorado.edu',
    name: 'University of Colorado Boulder',
    shortName: 'CU Boulder',
    nameZh: '科罗拉多大学博尔德分校',
    color: '#CFB87C',
    location: 'Boulder, CO',
    coverage: campusCoverage('boulder'),
    catalog: { colleges: 6, majors: 79 },
  },
  {
    slug: 'uiowa',
    domain: 'uiowa.edu',
    name: 'University of Iowa',
    shortName: 'Iowa',
    nameZh: '爱荷华大学',
    color: '#FFCD00',
    location: 'Iowa City, IA',
    coverage: campusCoverage('uiowa'),
    catalog: { colleges: 6, majors: 36 },
  },
  {
    slug: 'utk',
    domain: 'utk.edu',
    name: 'University of Tennessee, Knoxville',
    shortName: 'Tennessee',
    nameZh: '田纳西大学诺克斯维尔分校',
    color: '#FF8200',
    location: 'Knoxville, TN',
    coverage: campusCoverage('utk'),
    catalog: { colleges: 11, majors: 74 },
  },
  {
    slug: 'utdallas',
    domain: 'utdallas.edu',
    name: 'University of Texas at Dallas',
    shortName: 'UT Dallas',
    nameZh: '德克萨斯大学达拉斯分校',
    color: '#E87500',
    location: 'Richardson, TX',
    coverage: campusCoverage('utdallas'),
    catalog: { colleges: 6, majors: 39 },
  },
  {
    slug: 'asu',
    domain: 'asu.edu',
    name: 'Arizona State University',
    shortName: 'ASU',
    nameZh: '亚利桑那州立大学',
    color: '#8C1D40',
    location: 'Tempe, AZ',
    coverage: campusCoverage('asu'),
    catalog: { colleges: 10, majors: 82 },
  },
  {
    slug: 'iastate',
    domain: 'iastate.edu',
    name: 'Iowa State University',
    shortName: 'Iowa State',
    nameZh: '爱荷华州立大学',
    color: '#C8102E',
    location: 'Ames, IA',
    coverage: campusCoverage('iastate'),
    catalog: { colleges: 6, majors: 51 },
  },
  {
    slug: 'ucf',
    domain: 'ucf.edu',
    name: 'University of Central Florida',
    shortName: 'UCF',
    nameZh: '中佛罗里达大学',
    color: '#BA9B37',
    location: 'Orlando, FL',
    coverage: campusCoverage('ucf'),
    catalog: { colleges: 7, majors: 46 },
  },
  {
    slug: 'arizona',
    domain: 'arizona.edu',
    name: 'University of Arizona',
    shortName: 'Arizona',
    nameZh: '亚利桑那大学',
    color: '#AB0520',
    location: 'Tucson, AZ',
    coverage: campusCoverage('arizona'),
    catalog: { colleges: 12, majors: 86 },
  },
  {
    slug: 'houston',
    domain: 'uh.edu',
    name: 'University of Houston',
    shortName: 'Houston',
    nameZh: '休斯顿大学',
    color: '#C8102E',
    location: 'Houston, TX',
    coverage: campusCoverage('houston'),
    catalog: { colleges: 6, majors: 42 },
  },
  {
    slug: 'oregonstate',
    domain: 'oregonstate.edu',
    name: 'Oregon State University',
    shortName: 'Oregon State',
    nameZh: '俄勒冈州立大学',
    color: '#DC4405',
    location: 'Corvallis, OR',
    coverage: campusCoverage('oregonstate'),
    catalog: { colleges: 6, majors: 41 },
  },
  {
    slug: 'uky',
    domain: 'uky.edu',
    name: 'University of Kentucky',
    shortName: 'Kentucky',
    nameZh: '肯塔基大学',
    color: '#0033A0',
    location: 'Lexington, KY',
    coverage: campusCoverage('uky'),
    catalog: { colleges: 5, majors: 34 },
  },
  {
    slug: 'colostate',
    domain: 'colostate.edu',
    name: 'Colorado State University',
    shortName: 'Colorado State',
    nameZh: '科罗拉多州立大学',
    color: '#1E4D2B',
    location: 'Fort Collins, CO',
    coverage: campusCoverage('colostate'),
    catalog: { colleges: 6, majors: 55 },
  },
  {
    slug: 'utah',
    domain: 'utah.edu',
    name: 'University of Utah',
    shortName: 'Utah',
    nameZh: '犹他大学',
    color: '#BE0000',
    location: 'Salt Lake City, UT',
    coverage: campusCoverage('utah'),
    catalog: { colleges: 6, majors: 41 },
  },
  {
    slug: 'cincinnati',
    domain: 'uc.edu',
    name: 'University of Cincinnati',
    shortName: 'Cincinnati',
    nameZh: '辛辛那提大学',
    color: '#E00122',
    location: 'Cincinnati, OH',
    coverage: campusCoverage('cincinnati'),
    catalog: { colleges: 6, majors: 43 },
  },
  {
    slug: 'unl',
    domain: 'unl.edu',
    name: 'University of Nebraska–Lincoln',
    shortName: 'Nebraska',
    nameZh: '内布拉斯加大学林肯分校',
    color: '#D00000',
    location: 'Lincoln, NE',
    coverage: campusCoverage('unl'),
    catalog: { colleges: 6, majors: 44 },
  },
  {
    slug: 'lsu',
    domain: 'lsu.edu',
    name: 'Louisiana State University',
    shortName: 'LSU',
    nameZh: '路易斯安那州立大学',
    color: '#461D7C',
    location: 'Baton Rouge, LA',
    coverage: campusCoverage('lsu'),
    catalog: { colleges: 8, majors: 48 },
  },
  {
    slug: 'nyu',
    domain: 'nyu.edu',
    name: 'New York University',
    shortName: 'NYU',
    nameZh: '纽约大学',
    color: '#57068C',
    location: 'New York, NY',
    coverage: campusCoverage('nyu'),
    catalog: { colleges: 4, majors: 29 },
  },
  {
    slug: 'georgetown',
    domain: 'georgetown.edu',
    name: 'Georgetown University',
    shortName: 'Georgetown',
    nameZh: '乔治城大学',
    color: '#041E42',
    location: 'Washington, DC',
    coverage: campusCoverage('georgetown'),
    catalog: { colleges: 5, majors: 36 },
  },
  {
    slug: 'emory',
    domain: 'emory.edu',
    name: 'Emory University',
    shortName: 'Emory',
    nameZh: '埃默里大学',
    color: '#012169',
    location: 'Atlanta, GA',
    coverage: campusCoverage('emory'),
    catalog: { colleges: 3, majors: 36 },
  },
  {
    slug: 'uva',
    domain: 'virginia.edu',
    name: 'University of Virginia',
    shortName: 'UVA',
    nameZh: '弗吉尼亚大学',
    color: '#232D4B',
    location: 'Charlottesville, VA',
    coverage: campusCoverage('uva'),
    catalog: { colleges: 6, majors: 39 },
  },
  {
    slug: 'tufts',
    domain: 'tufts.edu',
    name: 'Tufts University',
    shortName: 'Tufts',
    nameZh: '塔夫茨大学',
    color: '#3E8EDE',
    location: 'Medford, MA',
    coverage: campusCoverage('tufts'),
    catalog: { colleges: 3, majors: 43 },
  },
  {
    slug: 'bc',
    domain: 'bc.edu',
    name: 'Boston College',
    shortName: 'BC',
    nameZh: '波士顿学院',
    color: '#8A100B',
    location: 'Chestnut Hill, MA',
    coverage: campusCoverage('bc'),
    catalog: { colleges: 4, majors: 38 },
  },
  {
    slug: 'amherst',
    domain: 'amherst.edu',
    name: 'Amherst College',
    shortName: 'Amherst',
    nameZh: '阿默斯特学院',
    color: '#3F1F69',
    location: 'Amherst, MA',
    coverage: campusCoverage('amherst'),
    catalog: { colleges: 5, majors: 36 },
  },
  {
    slug: 'swarthmore',
    domain: 'swarthmore.edu',
    name: 'Swarthmore College',
    shortName: 'Swarthmore',
    nameZh: '斯沃斯莫尔学院',
    color: '#862633',
    location: 'Swarthmore, PA',
    coverage: campusCoverage('swarthmore'),
    catalog: { colleges: 3, majors: 34 },
  },
  {
    slug: 'pomona',
    domain: 'pomona.edu',
    name: 'Pomona College',
    shortName: 'Pomona',
    nameZh: '波莫纳学院',
    color: '#0057B8',
    location: 'Claremont, CA',
    coverage: campusCoverage('pomona'),
    catalog: { colleges: 4, majors: 44 },
  },
  {
    slug: 'wellesley',
    domain: 'wellesley.edu',
    name: 'Wellesley College',
    shortName: 'Wellesley',
    nameZh: '韦尔斯利学院',
    color: '#0142a4',
    location: 'Wellesley, MA',
    coverage: campusCoverage('wellesley'),
    catalog: { colleges: 3, majors: 44 },
  },
  {
    slug: 'bowdoin',
    domain: 'bowdoin.edu',
    name: 'Bowdoin College',
    shortName: 'Bowdoin',
    nameZh: '鲍登学院',
    color: '#9C0F2E',
    location: 'Brunswick, ME',
    coverage: campusCoverage('bowdoin'),
    catalog: { colleges: 4, majors: 39 },
  },
  {
    slug: 'carleton',
    domain: 'carleton.edu',
    name: 'Carleton College',
    shortName: 'Carleton',
    nameZh: '卡尔顿学院',
    color: '#0b5091',
    location: 'Northfield, MN',
    coverage: campusCoverage('carleton'),
    catalog: { colleges: 4, majors: 50 },
  },
  {
    slug: 'cmc',
    domain: 'cmc.edu',
    name: 'Claremont McKenna College',
    shortName: 'CMC',
    nameZh: '克莱蒙特·麦肯纳学院',
    color: '#a42d36',
    location: 'Claremont, CA',
    coverage: campusCoverage('cmc'),
    catalog: { colleges: 4, majors: 26 },
  },
  {
    slug: 'middlebury',
    domain: 'middlebury.edu',
    name: 'Middlebury College',
    shortName: 'Middlebury',
    nameZh: '明德学院',
    color: '#0d395f',
    location: 'Middlebury, VT',
    coverage: campusCoverage('middlebury'),
    catalog: { colleges: 4, majors: 46 },
  },
  {
    slug: 'davidson',
    domain: 'davidson.edu',
    name: 'Davidson College',
    shortName: 'Davidson',
    nameZh: '戴维森学院',
    color: '#D42121',
    location: 'Davidson, NC',
    coverage: campusCoverage('davidson'),
    catalog: { colleges: 4, majors: 37 },
  },
  {
    slug: 'grinnell',
    domain: 'grinnell.edu',
    name: 'Grinnell College',
    shortName: 'Grinnell',
    nameZh: '格林内尔学院',
    color: '#ED1C24',
    location: 'Grinnell, IA',
    coverage: campusCoverage('grinnell'),
    catalog: { colleges: 4, majors: 36 },
  },
  {
    slug: 'colby',
    domain: 'colby.edu',
    name: 'Colby College',
    shortName: 'Colby',
    nameZh: '科尔比学院',
    color: '#002878',
    location: 'Waterville, ME',
    coverage: campusCoverage('colby'),
    catalog: { colleges: 4, majors: 48 },
  },
  {
    slug: 'hamilton',
    domain: 'hamilton.edu',
    name: 'Hamilton College',
    shortName: 'Hamilton',
    nameZh: '汉密尔顿学院',
    color: '#0C2340',
    location: 'Clinton, NY',
    coverage: campusCoverage('hamilton'),
    catalog: { colleges: 4, majors: 35 },
  },
  {
    slug: 'vassar',
    domain: 'vassar.edu',
    name: 'Vassar College',
    shortName: 'Vassar',
    nameZh: '瓦萨学院',
    color: '#951829',
    location: 'Poughkeepsie, NY',
    coverage: campusCoverage('vassar'),
    catalog: { colleges: 4, majors: 46 },
  },
  {
    slug: 'smith',
    domain: 'smith.edu',
    name: 'Smith College',
    shortName: 'Smith',
    nameZh: '史密斯学院',
    color: '#1e2b54',
    location: 'Northampton, MA',
    coverage: campusCoverage('smith'),
    catalog: { colleges: 4, majors: 46 },
  },
  {
    slug: 'wlu',
    domain: 'wlu.edu',
    name: 'Washington and Lee University',
    shortName: 'W&L',
    nameZh: '华盛顿与李大学',
    color: '#1F3D7B',
    location: 'Lexington, VA',
    coverage: campusCoverage('wlu'),
    catalog: { colleges: 4, majors: 41 },
  },
  {
    slug: 'colgate',
    domain: 'colgate.edu',
    name: 'Colgate University',
    shortName: 'Colgate',
    nameZh: '科尔盖特大学',
    color: '#821019',
    location: 'Hamilton, NY',
    coverage: campusCoverage('colgate'),
    catalog: { colleges: 4, majors: 41 },
  },
  {
    slug: 'wesleyan',
    domain: 'wesleyan.edu',
    name: 'Wesleyan University',
    shortName: 'Wesleyan',
    nameZh: '维思大学',
    color: '#BA0C2F',
    location: 'Middletown, CT',
    coverage: campusCoverage('wesleyan'),
    catalog: { colleges: 4, majors: 36 },
  },
  {
    slug: 'haverford',
    domain: 'haverford.edu',
    name: 'Haverford College',
    shortName: 'Haverford',
    nameZh: '哈弗福德学院',
    color: '#BA0C2F',
    location: 'Haverford, PA',
    coverage: campusCoverage('haverford'),
    catalog: { colleges: 4, majors: 26 },
  },
  {
    slug: 'bates',
    domain: 'bates.edu',
    name: 'Bates College',
    shortName: 'Bates',
    nameZh: '贝茨学院',
    color: '#881124',
    location: 'Lewiston, ME',
    coverage: campusCoverage('bates'),
    catalog: { colleges: 4, majors: 37 },
  },
  {
    slug: 'barnard',
    domain: 'barnard.edu',
    name: 'Barnard College',
    shortName: 'Barnard',
    nameZh: '巴纳德学院',
    color: '#002F6C',
    location: 'New York, NY',
    coverage: campusCoverage('barnard'),
    catalog: { colleges: 4, majors: 37 },
  },
  {
    slug: 'coloradocollege',
    domain: 'coloradocollege.edu',
    name: 'Colorado College',
    shortName: 'Colorado College',
    nameZh: '科罗拉多学院',
    color: '#EAB337',
    location: 'Colorado Springs, CO',
    coverage: campusCoverage('coloradocollege'),
    catalog: { colleges: 4, majors: 42 },
  },
  {
    slug: 'macalester',
    domain: 'macalester.edu',
    name: 'Macalester College',
    shortName: 'Macalester',
    nameZh: '麦卡莱斯特学院',
    color: '#01426A',
    location: 'Saint Paul, MN',
    coverage: campusCoverage('macalester'),
    catalog: { colleges: 4, majors: 36 },
  },
  {
    slug: 'kenyon',
    domain: 'kenyon.edu',
    name: 'Kenyon College',
    shortName: 'Kenyon',
    nameZh: '凯尼恩学院',
    color: '#4B2E84',
    location: 'Gambier, OH',
    coverage: campusCoverage('kenyon'),
    catalog: { colleges: 4, majors: 37 },
  },
  {
    slug: 'brynmawr',
    domain: 'brynmawr.edu',
    name: 'Bryn Mawr College',
    shortName: 'Bryn Mawr',
    nameZh: '布林茅尔学院',
    color: '#a51b00',
    location: 'Bryn Mawr, PA',
    coverage: campusCoverage('brynmawr'),
    catalog: { colleges: 4, majors: 37 },
  },
];

const BY_SLUG = new Map(SCHOOLS.map((s) => [s.slug, s]));

export function bySlug(slug: string): School | undefined {
  return BY_SLUG.get(slug);
}

export type SchoolDetection =
  | { kind: 'school'; school: School }
  | { kind: 'edu' }
  | { kind: 'none' };

/**
 * Live-as-you-type detection. Requires a non-empty local part so the
 * chip never fires on a bare "@illinois.edu". Subdomains of a known
 * school domain (cs.illinois.edu, eecs.berkeley.edu) match the school.
 */
export function detectSchoolFromEmail(email: string): SchoolDetection {
  const at = email.lastIndexOf('@');
  if (at < 1) return { kind: 'none' };
  const domain = email.slice(at + 1).trim().toLowerCase();
  if (!domain) return { kind: 'none' };
  for (const school of SCHOOLS) {
    if (domain === school.domain || domain.endsWith(`.${school.domain}`)) {
      return { kind: 'school', school };
    }
  }
  if (domain.length > 4 && domain.endsWith('.edu')) return { kind: 'edu' };
  return { kind: 'none' };
}
