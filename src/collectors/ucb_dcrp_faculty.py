"""Collector for UC Berkeley City & Regional Planning (CED) faculty.

A department config + bespoke listing parser over src.collectors.ucb_common. The
City & Regional Planning people page lives on the same College of Environmental
Design WordPress theme as Architecture, so the parsing mirrors
ucb_arch_faculty: each person is an `a[href=/people/<slug>]` wrapping a
`.people-listing` photo with the name in `div.font-bold` and the title in the
following div. The page mixes professors, lecturers, PhD students, advisors, and
staff, so only entries whose title names a Professor or Lecturer are kept
(emeritus faculty are then dropped by the shared title filter).
...
Directory: https://ced.berkeley.edu/academics/city-regional-planning/people

Usage:
    python -m src.collectors.ucb_dcrp_faculty            # fetch & preview
    python -m src.collectors.ucb_dcrp_faculty --no-enrich  # skip profile hop (fast)
    python -m src.collectors.ucb_dcrp_faculty --save     # merge into processed data
"""
...
DCRP_CONFIG = {
    "source": "ucb_dcrp_faculty",
    "name": "City & Regional Planning (College of Environmental Design)",
    "short": "DCRP",
    "url": "https://ced.berkeley.edu/academics/city-regional-planning/people",
    "base": "https://ced.berkeley.edu",
    "majors": ["City & Regional Planning", "Urban Planning", "Urban Studies"],
...
def _scrape_dcrp_faculty_list(soup: BeautifulSoup, base: str) -> list[dict]:
    """Parse the City & Regional Planning people page into [{name, url, title}].
    ...
    logger.info(f"  Found {len(faculty)} City & Regional Planning faculty")
    return faculty
...
def _enrich_dcrp_profiles(faculty: list[dict], config: dict) -> list[dict]:
...
def fetch_and_normalize(enrich: bool = True) -> list[dict]:
    """Scrape City & Regional Planning faculty and return normalized records.
    ...
    """
    soup = fetch_soup(DCRP_CONFIG["url"])
    if not soup:
        return []
    raw = dedup_by_profile_url(_scrape_dcrp_faculty_list(soup, DCRP_CONFIG["base"]))
    if enrich:
        raw = _enrich_dcrp_profiles(raw, DCRP_CONFIG)
    normalized = [n for n in (normalize_faculty(p, DCRP_CONFIG) for p in raw) if n]
    logger.info(f"Normalized {len(normalized)} City & Regional Planning opportunities")
    return normalized


if __name__ == "__main__":
    ucb_common.run_cli(DCRP_CONFIG, "UC Berkeley City & Regional Planning Faculty Collector",
