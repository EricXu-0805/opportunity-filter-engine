"""Collector for UC Berkeley Earth & Planetary Science (EPS) faculty.

A thin department config over src.collectors.ucb_common — the same config-only
path as ucb_chem_faculty. EPS uses the identical Open-Berkeley card variant:
each listing card is `div.node-openberkeley-person` with the name in an `<h2>`
and the profile href on a `/people/` link, and each profile exposes a mailto:
email (plus the email field) and a free-text `field-openberkeley-person-resint`
research block.
...
Directory: https://eps.berkeley.edu/people/faculty

Usage:
    python -m src.collectors.ucb_eps_faculty            # fetch & preview
    python -m src.collectors.ucb_eps_faculty --no-enrich  # skip profile hop (fast)
    python -m src.collectors.ucb_eps_faculty --save     # merge into processed data
"""
...
EPS_CONFIG = {
    "source": "ucb_eps_faculty",
    "name": "Department of Earth & Planetary Science",
    "short": "EPS",
    "url": "https://eps.berkeley.edu/people/faculty",
    "base": "https://eps.berkeley.edu",
    "majors": ["Earth & Planetary Science", "Geology", "Geophysics"],
...
def fetch_and_normalize(enrich: bool = True) -> list[dict]:
    return ucb_common.fetch_and_normalize(EPS_CONFIG, enrich=enrich)


if __name__ == "__main__":
    ucb_common.run_cli(EPS_CONFIG, "UC Berkeley Earth & Planetary Science Faculty Collector")
