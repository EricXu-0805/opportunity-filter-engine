...
refresh maintains.  Eligibility is strictly PER RECORD: an event is served iff
its own stored evidence validates (hashes, fingerprint, and event id must
deterministically reproduce — see ``validate_tracking_event_evidence``).
There is no global freshness gate and no release marker — a missing or empty
artifact is an honest ``available: false`` empty response, never an error, so
the feature works from the first verified change the pipeline ever writes.
...
def reset_tracking_cache() -> None:
    global _cache_signature, _cache_available, _cache_events_by_professor
    with _cache_lock:
...
def _load_events() -> tuple[bool, dict[str, list[dict]]]:
    global _cache_signature, _cache_available, _cache_events_by_professor
    signature = _path_signature()
    with _cache_lock:
        if signature == _cache_signature:
            return _cache_available, _cache_events_by_professor
...
async def professor_updates(body: ProfessorUpdatesRequest):
    ...
    available, by_professor = _load_events()
